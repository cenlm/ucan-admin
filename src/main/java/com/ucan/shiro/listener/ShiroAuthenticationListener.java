...
public class ShiroAuthenticationListener implements AuthenticationListener {
    ...
    public void onSuccess(AuthenticationToken token, AuthenticationInfo info) {
        ...
        log.info("session的缓存块的名称：" + shiroSessionDao.getActiveSessionsCacheName());
        log.info("缓存中还存活的session的sessionId:");
        HashSet<Serializable> sessionIds = new HashSet<Serializable>();
        shiroSessionDao.getActiveSessions().forEach(session -> {
